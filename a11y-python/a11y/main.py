...
from a11y.api.router import router
from a11y.api.v1.combined import _evict_old_jobs
...
logger = setup_logger(name="AC", tag="main")
...
async def lifespan(app: FastAPI):
    logger.info("a11y API starting up")
    ...
    try:
        from a11y.store import init_db
    ...
    # by A11Y_MAX_CONCURRENT_JOBS. Retention sweep (P1) prunes old runs+assets.
    dispatcher_task = None
    retention_task = None
    try:
        from a11y.api.v1.combined.dispatcher import run_dispatcher
        from a11y.store.retention import run_retention_loop
    ...
    try:
        ...
        try:
            from a11y.crawler.browser_pool import shutdown_pool

            await shutdown_pool()
        except Exception:  # noqa: BLE001
            logger.exception("browser pool shutdown failed during lifespan teardown")
        try:
            from a11y.store.cpu_pool import shutdown_pool as shutdown_cpu_pool

            shutdown_cpu_pool()
        except Exception:  # noqa: BLE001
            logger.exception("CPU pool shutdown failed")
        try:
            from a11y.store import shutdown_db

            shutdown_db()
        except Exception:  # noqa: BLE001
            logger.exception("SQLite store shutdown failed")
        logger.info("a11y API shutting down")
