"""
a11y/utils/stage_timing.py
===========================
Low-level per-(page, stage, sub_stage, rule) timing logger.

Where the existing :mod:`a11y.utils.run_timing` writes one **aggregate** block
per run and :mod:`a11y.utils.crawler_timing` writes one row per crawler
...
Two files per run, both under ``a11y-python/logs/timings/``:
...
Override the directory with ``$A11Y_STAGE_TIMING_DIR``. Disable entirely with
...
*   Every write is wrapped in try/except. A logging failure must never break
    an audit. Errors are logged via the standard a11y logger.
...
logger = setup_logger(name="AC", tag="stage_timing")

_DISABLE_ENV = "A11Y_STAGE_TIMING_DISABLE"
_DIR_ENV = "A11Y_STAGE_TIMING_DIR"
...
def _timings_dir() -> Path:
    ...
    # Sibling of run_timings.log / AC_*.log for consistency.
...
def _files_enabled() -> bool:
    """JSONL/summary files are opt-out (P3). Set A11Y_TELEMETRY_FILES=0 in prod
    to keep telemetry in SQLite only and avoid unbounded log-file growth."""
    return os.environ.get("A11Y_TELEMETRY_FILES", "1") != "0"
...
def emit_summary(run_id: str) -> Optional[Path]:
    ...
    try:
        ...
        lines: List[str] = []
        lines.append(f"# a11y stage timing summary — run_id={run_id}")
