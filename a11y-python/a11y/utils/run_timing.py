...
appends one human-readable block to a persistent log file:
    ...
    a11y-python/logs/run_timings.log      (override with $A11Y_RUN_TIMING_LOG)
...
logger = setup_logger(name="AC", tag="run_timing")

# Single append lock — concurrent jobs (up to A11Y_MAX_CONCURRENT_JOBS) may
...
def _log_path() -> Path:
    override = os.getenv("A11Y_RUN_TIMING_LOG")
    if override:
        return Path(override)
    # Same logs/ dir the rotating app logger uses (a11y-python/logs/).
...
def log_run_timing(
    *,
    ...
    try:
        ...
        path = _log_path()
        if os.environ.get("A11Y_TELEMETRY_FILES", "1") != "0":
