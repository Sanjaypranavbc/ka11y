...
Default OFF. Enabled with ``A11Y_UNIFIED_CRAWL=1``; the runner then swaps the
...
axe.min.js resolution order:
  1. ``$A11Y_AXE_MIN_JS``
  2. vendored ``a11y/crawler/vendor/axe.min.js``
  3. dev fallback ``../a11y-node/node_modules/axe-core/axe.min.js``
"""
...
logger = setup_logger(name="AC", tag="axe_runner")
...
def is_unified_enabled() -> bool:
    return os.environ.get("A11Y_UNIFIED_CRAWL", "0") == "1"


def resolve_axe_js() -> Optional[Path]:
    override = os.environ.get("A11Y_AXE_MIN_JS")
    candidates = [
        Path(override) if override else None,
        Path(__file__).resolve().parent / "vendor" / "axe.min.js",
        Path(__file__).resolve().parents[3] / "a11y-node" / "node_modules" / "axe-core" / "axe.min.js",
    ]
    for c in candidates:
        if c and c.is_file():
            return c
...
def map_axe_results(
    ...
    """Map a raw ``axe.run`` result to the flat finding shape used across a11y.

    Mirrors ``a11y-node/src/utils/axeResultMapper.js`` (the fields
...
async def _run_axe_on_url(
    ...
    try:
        from a11y.api.v1.combined.routes import build_ssrf_route_handler
...
async def run_axe_unified(
    ...
    *,
    ...
    if axe_js is None:
        raise RuntimeError(
            "A11Y_UNIFIED_CRAWL is on but axe.min.js was not found. Set "
            "A11Y_AXE_MIN_JS or vendor a11y/crawler/vendor/axe.min.js."
        )
