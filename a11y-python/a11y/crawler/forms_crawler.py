...
class AsyncFormCrawler:
    ...
    TRIGGER_VALIDATION_JS = r"""() => {
        ...
        forms.forEach((form) => {
            if (form.__a11ySubmitGuard) return;
            form.__a11ySubmitGuard = true;
    ...
    async def crawl(self) -> List[FormInputData]:
        # Sprint-2 (#9): lease a context from the shared BrowserPool instead
