"""
a11y/crawler/rendered_layout_crawler.py
=========================================
...
from a11y.crawler.context_factory import new_crawler_context
from a11y.crawler.cookie_handler import handle_cookies
from a11y.accessibility.rendered.geometry import rect_from_dict
from a11y.accessibility.rendered.heuristics import compute_obscuration
from a11y.accessibility.rendered.models import (
    FocusStep,
    HoverInteractionResult,
    PageSnapshot,
)
from a11y.accessibility.rendered.snapshot_collector import collect_snapshot
from a11y.accessibility.rendered.stabilizer import stabilize
...
logger = setup_logger(name="AC", tag="rendered_layout")
...
def run_all_evaluators(
    ...
    for raw in raw_list:
        ...
        records = []
        if run_resize_text:
            from a11y.accessibility.rendered.evaluators import evaluate_resize_text
        ...
        if run_reflow:
            from a11y.accessibility.rendered.evaluators import evaluate_reflow

            records.extend(evaluate_reflow(_snap("reflow_320")))

        if run_text_spacing:
            from a11y.accessibility.rendered.evaluators import evaluate_text_spacing
        ...
        if run_orientation:
            from a11y.accessibility.rendered.evaluators import evaluate_orientation
