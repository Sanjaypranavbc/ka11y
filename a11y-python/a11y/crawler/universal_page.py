...
from a11y.crawler.navigation import navigate_with_resilience, NavigationError
...
from a11y.accessibility.pipeline.extractors.element_context_extractor import (
    ElementContextExtractor,
)
from a11y.accessibility.pipeline.extractors.semantic_relationship_engine import (
    SemanticRelationshipEngine,
)
from a11y.accessibility.pipeline.runners.interaction_state_runner import (
    InteractionStateRunner,
)

logger = setup_logger(name="AC", tag="universal_page")
...
_UNIVERSAL_PARALLEL_PAGES = max(
    1, int(os.environ.get("A11Y_UNIVERSAL_PARALLEL_PAGES", "4"))
)
...
_COMBINED_EXTRACT_JS = r"""(frameMeta) => {
    ...
    if (!window._a11yIdCounter) window._a11yIdCounter = 1;
    function getA11yId(el) {
        if (!el || !el.getAttribute || !el.setAttribute) return null;
        if (!el.getAttribute('data-a11y-id')) {
            el.setAttribute('data-a11y-id', 'k-' + (window._a11yIdCounter++));
        }
    ...
    function metaFor(el) {
        return {
            ...
            element_ref_id: getA11yId(el) || undefined,
        };
    }
    ...
    (function extractMovingContent() {
            ...
            for (const container of containers) {
                queryShadow(container, 'button,[role="button"],a').forEach(btn => {
                    ...
                    if (/pause|stop|一時停止|停止|止める|再生停止/.test(text)) {
                        throw new Error('__A11Y_HAS_PAUSE__');
                    }
                });
            }
            return false;
        ...
        function hasPauseButton(el) {
            try {
                ...
                return String(err.message || err) === '__A11Y_HAS_PAUSE__';
            }
        }
