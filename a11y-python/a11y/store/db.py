...
logger = setup_logger(name="AC", tag="store.db")
...
def _default_db_path() -> str:
    override = os.getenv("A11Y_DB_PATH")
    if override:
        return override
    # Default lives next to the rotating app logs so a single mounted volume
    # (logs/) captures both. Compose can override with A11Y_DB_PATH=/data/...
    log_dir = Path(__file__).resolve().parent.parent.parent / "logs"
    return str(log_dir / "a11y.db")
    ...
    def start(self) -> None:
        ...
        with self._start_lock:
            ...
            self._writer = threading.Thread(
                target=self._writer_loop, name="a11y-db-writer", daemon=True
            )
            self._writer.start()
            self._started.set()
