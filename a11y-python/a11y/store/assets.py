"""
a11y/store/assets.py
=====================
Content-addressed asset store (Decision #2: bytes on disk, metadata in DB).

``put_asset`` hashes the bytes, writes them under ``A11Y_ASSET_DIR`` at a
...
from a11y.store.db import get_db

logger = setup_logger(name="AC", tag="store.assets")


def asset_dir() -> Path:
    override = os.getenv("A11Y_ASSET_DIR")
    if override:
