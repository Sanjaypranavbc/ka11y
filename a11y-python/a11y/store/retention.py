"""
a11y/store/retention.py
=======================
Durable-store retention sweep (P1). Replaces the in-memory TTL eviction's role
for the DB: periodically delete runs older than ``A11Y_RUN_RETENTION_DAYS``
...
from a11y.store.assets import prune_run_assets

logger = setup_logger(name="AC", tag="store.retention")

_SWEEP_INTERVAL_SECONDS = int(os.getenv("A11Y_RETENTION_SWEEP_SECONDS", "3600"))


async def run_retention_loop() -> None:
    retention_days = int(os.getenv("A11Y_RUN_RETENTION_DAYS", "30"))
    while True:
        try:
