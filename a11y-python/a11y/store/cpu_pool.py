...
logger = setup_logger(name="AC", tag="store.cpu_pool")


def _default_workers() -> int:
    override = os.getenv("A11Y_CPU_WORKERS")
    if override:
        try:
...
def get_pool() -> Optional[ProcessPoolExecutor]:
    ...
    Opt-in: the pool is only created when ``A11Y_CPU_WORKERS`` is set to a
    ...
    multi-core parallelism on the box by exporting e.g. ``A11Y_CPU_WORKERS=6``.
    """
    global _pool
    raw = os.getenv("A11Y_CPU_WORKERS")
    if not raw or raw == "0":
        return None
    if _pool is None:
        try:
