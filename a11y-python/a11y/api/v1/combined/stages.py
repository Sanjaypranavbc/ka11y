"""
a11y/api/v1/combined/stages.py
=================================
...
from a11y.utils.crawler_settings import (
...
from a11y.accessibility.pipeline.pipeline_stage import (
    _run_pipeline_stage as _real_run_pipeline_stage,
)


from a11y.utils.step_logger import ExecutionStepLogger
from a11y.utils.crawler_timing import time_crawler
...
_CRAWL_PER_PAGE_SECONDS = float(os.environ.get("A11Y_IMAGE_CRAWL_PER_PAGE_SECONDS", "20"))
...
_CRAWL_TIMEOUT_CEILING = float(os.environ.get("A11Y_IMAGE_CRAWL_TIMEOUT_CEILING", "600"))
...
_SNAPSHOT_URLS_WAIT_SECONDS = float(
    os.environ.get("A11Y_SNAPSHOT_URLS_WAIT_SECONDS", str(_STAGE_TIMEOUT_SECONDS))
)
...
_NODE_HTTP_BASE_TIMEOUT_SECONDS = float(
    os.environ.get("A11Y_NODE_HTTP_BASE_TIMEOUT_SECONDS", "60")
)
_NODE_HTTP_PER_PAGE_TIMEOUT_SECONDS = float(
    os.environ.get("A11Y_NODE_HTTP_PER_PAGE_TIMEOUT_SECONDS", "75")
)
# Hard ceiling — keep at/under the runner's A11Y_JOB_TIMEOUT_SECONDS (1800 s)
...
_NODE_HTTP_TIMEOUT_CAP_SECONDS = float(
    os.environ.get("A11Y_JOB_TIMEOUT_SECONDS", "1800")
)
...
_HEAVY_STAGE_CONCURRENCY = int(os.environ.get("A11Y_HEAVY_STAGE_CONCURRENCY", "2"))
    ...
    *,
    ...
    if snapshot_urls_event is not None and snapshot_urls_container is not None and not discovered_urls:
        try:
            ...
            logger.error(
                ...
                f"Raise A11Y_SNAPSHOT_URLS_WAIT_SECONDS if the snapshot is "
    ...
    try:
        ...
        raise RuntimeError(
            ...
            f"A11Y_NODE_HTTP_PER_PAGE_TIMEOUT_SECONDS / A11Y_JOB_TIMEOUT_SECONDS "
...
async def _stage_image_audit(
    ...
    try:
        from a11y.accessibility.rules.non_text.alttext import (
            AltTextAccessibilityAuditor,
        )
        from a11y.crawler.crawler import (
            AsyncImageCrawler,
            ImageCrawlerNavigationError,
        )
        from a11y.text_detector.text_detector import (
            OCRPreprocessing,
            TextClassification,
        )
...
async def _stage_form_audit(
    ...
    try:
        from a11y.accessibility.rules.forms.form_auditor import (
...
async def _stage_sensory_audit(
    ...
    from a11y.api.v1.combined.stage_events import (
        _stage_complete,
        _stage_error_and_warn,
        _stage_start,
    )
    from a11y.api.v1.combined.findings import _sensory_to_findings  # noqa: F401
    ...
    try:
        from a11y.accessibility.rules.non_text.sensory_auditor import (
            SensoryCharacteristicsAuditor,
        )
        from a11y.crawler.sensory_crawler import AsyncSensoryCrawler
...
async def _stage_label_in_name(
    ...
    try:
        from a11y.accessibility.rules.input_modalities.label_in_name_auditor import (
            LabelInNameAuditor,
        )
...
async def _stage_pause_stop_hide(
    ...
    try:
        from a11y.accessibility.rules.timing.pause_stop_hide_auditor import (
...
async def _stage_rendered_layout_audit(
    ...
    try:
        from a11y.crawler.rendered_layout_crawler import (
...
async def _stage_media_audit(
    ...
    try:
        from a11y.accessibility.rules.media.media_auditor import MediaAuditor
...
async def _load_universal_snapshot(
    *,
    ...
    from a11y.crawler.snapshot_normalizer import SnapshotNormalizer
    from a11y.crawler.policy import CrawlPolicy
...
async def _stage_consistent_navigation(
    ...
    try:
        from a11y.accessibility.rules.navigation.consistent_nav import analyze_wcag_323
...
async def _stage_consistent_identification(
    ...
    try:
        from a11y.accessibility.rules.navigation.consistent_id import analyze_wcag_324
...
async def _stage_unusual_words(
    ...
    try:
        from a11y.accessibility.rules.language.unusual_words import analyze_wcag_313
...
async def _stage_section_headings(
    ...
    try:
        from a11y.accessibility.rules.rendered.section_headings import analyze_wcag_2410
