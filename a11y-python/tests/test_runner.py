"""
Unit tests for a11y/api/v1/combined/runner.py
...
async def test_python_result_non_tuple_does_not_raise():
    ...
    with patch(
        "a11y.api.v1.combined.runner._run_python_stages",
        new=AsyncMock(return_value="not-a-tuple"),
    ), patch(
        "a11y.api.v1.combined.runner._call_node_flat",
        new=AsyncMock(return_value=[]),
    ), patch(
        "a11y.api.v1.combined.runner._build_report",
        return_value={
        ...
        "a11y.api.v1.combined.runner.load_config",
        return_value={"input": {"output_dir": "/tmp/a11y-runner-test"}},
    ):
        from a11y.api.v1.combined.runner import _run_job
