...
def test_image_crawler_navigation_error_dns_message_is_frontend_clear():
    from a11y.crawler.crawler import ImageCrawlerNavigationError
...
def test_build_image_audit_report_surfaces_all_audited_images():
    from a11y.api.v1.combined.findings import _build_image_audit_report
...
async def test_stage_image_audit_surfaces_dns_resolution_warning():
    from a11y.api.v1.combined import store
    from a11y.api.v1.combined.stages import _stage_image_audit
    from a11y.crawler.crawler import ImageCrawlerNavigationError
    ...
    class DummyCrawler:
        def __init__(
            ...
            self.job_id = job_id
            self.images_data = []
            self.output_dir = "/tmp/a11y-image-audit-test"
    ...
    with patch("a11y.crawler.crawler.AsyncImageCrawler", DummyCrawler), patch(
        "a11y.accessibility.rules.non_text.alttext.AltTextAccessibilityAuditor",
        DummyAuditor,
    ), patch(
        "a11y.text_detector.text_detector.OCRPreprocessing",
        DummyOCR,
    ), patch(
        "a11y.text_detector.text_detector.TextClassification",
