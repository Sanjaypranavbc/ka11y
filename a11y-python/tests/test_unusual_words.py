...
from a11y.accessibility.rules.language import unusual_words
from a11y.api.v1.combined.findings import _unusual_words_to_findings
...
async def test_analyze_wcag_313_runs_with_mocked_nlp():
    ...
    with patch("a11y.accessibility.rules.language.unusual_words.leased_context") as mock_lease:
        mock_ctx = AsyncMock()
        mock_page = AsyncMock()
        ...
        with patch("a11y.accessibility.rules.language.unusual_words.extract_candidate_terms", return_value=(["cryptography"], mock_doc)):
