...
from a11y.store import db as dbm
...
def isolated_db(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("A11Y_DB_PATH", str(db_path))
    monkeypatch.setenv("A11Y_ASSET_DIR", str(asset_path))
    monkeypatch.setenv("A11Y_TELEMETRY_FILES", "0")
    dbm.shutdown_db()
...
async def test_asset_put_get_roundtrip(isolated_db):
    from a11y.store.assets import get_asset, put_asset
...
async def test_dispatcher_executes_queued_run(isolated_db, monkeypatch):
    ...
    from a11y.api.v1.combined import dispatcher
    from a11y.api.v1.combined import runner as runner_mod
    from a11y.api.v1.combined.store import _jobs
...
async def test_get_after_eviction_reads_from_db(isolated_db):
    ...
    from a11y.api.v1.combined.routes import get_combined_audit
    from a11y.api.v1.combined.store import _jobs
...
async def test_history_endpoint(isolated_db):
    from a11y.api.v1.combined.routes import list_combined_history
...
async def test_cancel_endpoint(isolated_db):
    from a11y.api.v1.combined.routes import cancel_combined_audit
    from a11y.api.v1.combined.store import _jobs
...
async def test_admin_metrics(isolated_db):
    from a11y.api.v1.assets import admin_metrics
...
def test_axe_mapper_shapes_findings():
    ...
    from a11y.crawler.axe_runner import map_axe_results
...
async def test_register_report_assets_rewrites_urls(isolated_db, tmp_path):
    ...
    from a11y.store.assets import register_report_assets
...
async def test_crawler_timing_mirrors_to_db(isolated_db, tmp_path, monkeypatch):
    ...
    from a11y.utils import crawler_timing
...
async def test_run_timing_mirrors_to_run_events(isolated_db):
    ...
    from a11y.utils.run_timing import log_run_timing
...
async def test_merge_findings_via_run_cpu(isolated_db):
    ...
    from a11y.api.v1.combined.runner import _merge_findings
    from a11y.store.cpu_pool import run_cpu
...
def test_build_report_stamps_finding_ids_and_manual_review():
    from a11y.api.v1.combined.report import _build_report
...
def test_apply_reviews_recomputes_score():
    from a11y.api.v1.combined.report import _build_report, apply_reviews
...
async def test_review_endpoint_updates_effective_score(isolated_db):
    from a11y.api.v1.combined.report import _build_report
    from a11y.api.v1.combined.routes import (
...
async def test_rerun_creates_new_run_from_stored_params(isolated_db, monkeypatch):
    ...
    from a11y.api.v1.combined import routes
