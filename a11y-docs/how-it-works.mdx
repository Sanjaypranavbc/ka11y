---
title: "How a11y Works"
description: "A plain-language guide to a11y's two-engine audit model, WCAG rule coverage, and combined reporting"
---

## What happens when you submit a URL

You provide a URL. a11y launches two specialised engines — one that analyses the **rendered visual page** and one that analyses the **underlying code structure**. Both engines crawl the same URL in parallel (unless the Node engine is disabled via the `run_node_audit` flag), then their findings are merged into a single unified report.

```mermaid
graph LR
    U(["You submit a URL"]) --> K["a11y Platform"]
...
The two engines complement each other — neither alone gives the full picture. Together they cover **37 WCAG 2.1 / 2.2 success criteria** across conformance levels A, AA, and AAA. If you only need visual/multimodal analysis, the Node engine can be toggled off to reduce audit duration.
...
The Node engine also visits your URL with a real browser (Puppeteer), but instead of looking at visual output it **inspects the page's code and DOM**. It injects the industry-standard axe-core library (used by Deque, Microsoft, Google, and others), which runs over 100 automated WCAG rule checks against the page structure. On top of axe-core, a11y adds 28 of its own custom checks for rules axe-core doesn't cover — including media captions, audio-only transcripts, and a CSS `background-image` content scan that flags meaningful backgrounds with no accessible name.
...
**a11y custom checks on top of axe-core:**
...
| 2.5.1 Pointer Gestures | Functionality using multipoint or path-based gestures has a single-pointer alternative | A |
...
| 2.5.4 Motion Actuation | Actions triggered by device motion have UI alternatives and an off-switch | A |
...
After both engines complete, a11y merges their findings using a three-step process.
...
**Why deduplication matters:** Some rules — such as 1.1.1 (alt text) or 1.3.4 (orientation) — are checked by both engines. Without deduplication, the same issue would appear twice in the report. a11y uses a fingerprint of the element and WCAG criterion to detect overlaps, so each issue appears exactly once.
...
- A single **compliance score** (0–100) — the pass rate `passes / (passes + violations)`, with needs-review excluded
...
**Per-page reporting (multi-page crawls).** When you raise **Max Depth above 0**,
a11y crawls multiple same-domain pages and attributes every finding to the page
it was found on. The report stays aggregated at the top (the headline counts and
score cover the whole crawl), and a `pages` array carries the same findings grouped
per page — each with its own score and severity breakdown, sorted worst-first. In
the dashboard this surfaces as a **page selector + per-page summary bar** on the
Violations, Needs Review, Passes, and Image Visualiser views; the selector is hidden
for single-page audits. See [Output Format → Per-page breakdown](/internals/output-format#per-page-breakdown-pages).

---

## Language support

a11y ships with WCAG rule descriptions in **multiple languages**. The `lang` parameter on any audit request selects the language for criterion names, severity labels, and error descriptions in the report.
...
Because a full audit can take 30–90 seconds (more pages = more time), a11y uses an **asynchronous job model** with real-time progress streaming.

```mermaid
sequenceDiagram
    participant C as Your Application
    participant K as a11y Platform
