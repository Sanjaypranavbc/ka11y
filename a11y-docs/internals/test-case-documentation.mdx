...
description: "Every test class and function in a11y-python/tests with full reasoning for techniques and inputs."
---

**Scope:** `a11y-python/tests/` — every test class, every test function, with full reasoning for techniques and inputs.
**Date:** 2026-03-23
...
- **Why:** All violation messages in a11y are prefixed with the WCAG criterion number (`"2.2.2: ..."`). This allows the frontend to group and sort by criterion without parsing unstructured text.
...
- **Why:** The `axe_would_catch` field distinguishes findings that axe-core would also detect from those unique to a11y's crawler. This is used by the dashboard to quantify a11y's unique coverage ("X violations that axe would have missed"). If this field is dropped, the coverage stats are wrong.
...
- **Why:** `axe_would_miss` = count of elements where `axe_would_catch=False`. This is a unique metric that quantifies the value a11y adds over running axe-core alone. An incorrect count here would misrepresent a11y's unique detection capability.
