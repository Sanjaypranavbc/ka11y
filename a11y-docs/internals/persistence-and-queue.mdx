...
description: "The durable SQLite store, crash-safe job queue, content-addressed asset store, and DB-backed telemetry that make a11y production-ready"
---
...
a11y now has a **durable store** (single-box SQLite, WAL) that is the source of
...
1. **On boot** the dispatcher runs `repo.requeue_running()` — any run left
   ...
   `A11Y_MAX_ATTEMPTS`). Interrupted audits **resume automatically**.
2. It drains `queued` rows **FIFO**, bounded by `A11Y_MAX_CONCURRENT_JOBS`.
...
| `POST /combined/{id}/rerun` | Re-audit a past run with its exact stored parameters — returns a **new** `job_id` (original preserved). Use to refresh a result after an engine change without re-entering the URL/toggles. |
| `GET /api/v1/assets/{id}` | Content-addressed asset serving. |
| `GET /api/v1/admin/metrics` | Telemetry rollups for ops. |

See [Environment Variables](/deployment/environment) for `A11Y_DB_PATH`,
`A11Y_ASSET_DIR`, retention, queue, CPU-pool, and the unified-crawl flag.

## Process pool & unified crawl (opt-in)

- `A11Y_CPU_WORKERS=N` (>0) routes pure-CPU work (report compression, finding
...
- `A11Y_UNIFIED_CRAWL=1` runs axe-core **inside the Playwright page** the
