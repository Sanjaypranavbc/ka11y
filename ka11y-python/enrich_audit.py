...
def enrich_violations(
    ...
    for i, chunk in enumerate(chunks):
        ...
        # One span per batch, parenting the LLM span the google-genai
        ...
        with traced_span(
            ...
            try:
                ...
                set_span_output(
                    span,
                    {"requested": len(chunk), "enriched": len(enriched_by_id)},
                )
                ...
                totals["failures"] += 1
                totals["retries"] += 1
                # The batch failure is swallowed below so the run continues on
                # static reason/fix — mark the span failed explicitly, since
                # OTel only does that for exceptions that escape the span.
...
def run_enrichment(
    ...
    mutate_in_place: when False (default, used by the CLI and any caller that
        still wants its own report object left untouched), enrichment runs
        against a deepcopy. Pipeline callers that want the dynamic fields to
        actually reach the report object they hand off downstream (DB save,
        websocket broadcast, API response) should pass True - the *same*
        finding dicts inside `report["violations"]` (and therefore the
        matching entries under report["pages"][i]["violations"], which the
        report builder stamps as shared references) are enriched directly.

    session_id: optional id (the pipeline passes the audit job id) that groups
    ...
    report_copy = report if mutate_in_place else deepcopy(report)
    ...
    # Parent span for the whole run: the per-batch spans (and the LLM spans
    ...
    with traced_span(
        ...
        session_id=session_id,
        metadata={"report_file": report_filename},
    ) as run_span:
        enrich_violations(
            violations,
            client,
...
def enrich_report_in_pipeline(
    ...
    """Entry point for pipeline/orchestrator callers (e.g.
    ka11y/api/v1/combined/runner.py). Thin wrapper around run_enrichment()
    with mutate_in_place=True, so *report*'s own violation dicts gain
    dynamic_reason / dynamic_suggested_fix / user_impact / confidence
    in place - the caller doesn't need to swap in a returned copy for the
    dynamic fields to reach whatever it does with `report` next (DB save,
    broadcast, API response). Also still writes enriched_report.json +
    token_usage.json to output_dir. Returns the token_usage dict.
    ...
    Japanese instead of defaulting to English. Pass session_id=job_id to group
    the run's Arize spans under that audit job."""
...
def main() -> int:
    ...
    # Spans are exported on a batch timer; a one-shot CLI run would otherwise
    # exit with the last batch still queued.
    flush_tracing()
