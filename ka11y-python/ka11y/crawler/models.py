...
class ImageMetadata(BaseModel):
    ...
    def compute_violations(self) -> "ImageMetadata":
        ...
        if (self.in_link or self.in_button or self.has_onclick):
            has_name = bool(
                (self.alt and self.alt.strip()) or
                self.aria_label or
                self.aria_labelledby or
                self.link_text or
                self.button_text
            )
            if not has_name:
        ...
        if (self.has_onclick or self.cursor_style == "pointer") \
                and not self.in_link and not self.in_button:
