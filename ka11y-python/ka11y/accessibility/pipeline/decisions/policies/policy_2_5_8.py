...
class Policy258(WCAGPolicy):
    ...
    def evaluate(self, element: ElementContext) -> RuleVerdict:
        if not element.interaction.is_focusable and element.semantics.tag_name not in (
            "button",
            "a",
            "input",
        ):
            return self._not_applicable(
                element,
                "not_interactive",
                "Target size minimum only applies to interactive elements.",
            )

        # 1. Exemptions
        if (
            "p" in element.semantics.parent_tags
            or element.visual.computed_styles.get("display") == "inline"
        ):
            return self._pass(
                element,
                "inline_exception",
                "Target is an inline link within a text block.",
            )
        ...
        if w >= MIN_TARGET_SIZE_PX and h >= MIN_TARGET_SIZE_PX:
            return self._pass(
                element,
                "size_meets_minimum",
                f"Target bounding box ({w}x{h}px) meets 24x24px minimum.",
        ...
        if element.interaction.adjacent_spacing_px >= MIN_TARGET_SPACING_PX:
            return self._pass(
                element,
                "spacing_exception",
                "Target is undersized but meets the 24px combined spacing exception.",
                reason_params=size_params,
            )
        ...
        return self._fail(
            element,
            "undersized_target",
            f"Target is undersized ({w}x{h}px) and lacks sufficient adjacent spacing.",
            reason_params=size_params,
        )
