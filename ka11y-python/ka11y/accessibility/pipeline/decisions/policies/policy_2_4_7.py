...
class Policy247(WCAGPolicy):
    ...
    def evaluate(self, element: ElementContext) -> RuleVerdict:
        ...
        if element.interaction.has_focus_ring:
            return self._pass(
                element,
                "focus_indicator_present",
                "Element presents a visual change when focused via keyboard.",
            )
        ...
        return self._fail(
            element,
            "no_focus_indicator",
            "Element has no visible focus indicator (no outline, box-shadow, or color change).",
        )
