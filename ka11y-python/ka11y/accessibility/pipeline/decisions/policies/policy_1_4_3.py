...
class Policy143(WCAGPolicy):
    ...
    def evaluate(self, element: ElementContext) -> RuleVerdict:
        if (
            not element.accessible_name
            and not element.visual.ocr_text
            and not element.visual.visible_label_text
        ):
            return self._not_applicable(element, "no_text", "Element contains no text.")

        # Exemptions: Logos, Inactive UI, Decorative
        if (
            element.visual.cv_classification in ("logo", "decorative")
            or element.semantics.is_disabled
        ):
            return self._pass(
                element,
                "contrast_exempt",
                "Logos, decorative, or disabled elements are exempt from contrast requirements.",
            )
        ...
        if "rgba" in bg_color and bg_color.endswith(", 0)"):
            return self._needs_review(
                element,
                "transparent_bg",
                "Background is transparent. Cannot compute text contrast statically.",
            )
        ...
        if result["passes"]:
            return self._pass(
                element,
                "contrast_sufficient",
                f"Contrast {result['ratio']}:1 meets the {result['threshold']}:1 minimum.",
                evidence,
            )
        ...
        return self._fail(
            ...
            f"Contrast {result['ratio']}:1 fails the {result['threshold']}:1 minimum requirement.",
            evidence,
        )
