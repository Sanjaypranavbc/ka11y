    ...
    def evaluate(self, element: ElementContext) -> RuleVerdict:
        ...
        if not is_ui_component and element.visual.cv_classification != "icon":
            return self._not_applicable(
                element,
                "not_ui_component",
                "Element is not an interactive UI component or graphical object.",
            )
        ...
        if element.semantics.is_disabled:
            return self._pass(
                element,
                "inactive_exempt",
                "Disabled UI components are exempt from contrast requirements.",
            )
        ...
        if bg_color.endswith(", 0)") and border_color.endswith(", 0)"):
            return self._needs_review(
                ...
                "Component lacks explicit border or background. Cannot compute boundary contrast automatically.",
            )

        return self._needs_review(
            element,
            "boundary_contrast_review",
            "Visual boundary contrast requires visual context/screenshot resolution.",
        )
