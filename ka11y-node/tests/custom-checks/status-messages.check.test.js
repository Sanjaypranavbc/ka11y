...
describe('status-messages.check (WCAG 4.1.3)', () => {
  ...
  test('fails when forms exist but no live regions', async () => {
    const page = makePage({ liveRegionCount: 0, formCount: 2, hasAlerts: false, hasPolite: false });
    const result = await run(page);
  ...
  test('incomplete when no forms and no live regions', async () => {
    const page = makePage({ liveRegionCount: 0, formCount: 0, hasAlerts: false, hasPolite: false });
    const result = await run(page);
    expect(result.rules[0].status).toBe('incomplete');
  });

  test('ruleId is custom-status-messages', async () => {
    const page = makePage({ liveRegionCount: 1, formCount: 0, hasAlerts: false, hasPolite: true });
    const result = await run(page);
