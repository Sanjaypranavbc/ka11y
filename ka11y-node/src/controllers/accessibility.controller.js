  ...
  */
  async analyseUrlFlat(req, res) {
    const { url, level = 'AA', lang = 'en', successCriteriaId = null,
            runAxe = true, runAccessLint = true } = req.body;
    ...
    try {
      ...
      const filter = successCriteriaId ?? null;
      this._logger.info(
        `analyseUrlFlat start url=${url} level=${wcagLevel} lang=${safeLang} successCriteriaId=${filter ?? 'none'} runAxe=${runAxe} runAccessLint=${runAccessLint}`
      );
      const findings = await this._service.analyseUrlFlat(url, wcagLevel, safeLang, filter, !!runAxe, !!runAccessLint);
