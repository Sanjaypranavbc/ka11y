...
function evaluatePage(tokens, confirmSrc, reuseSrc, keywordPairs) {
  ...
  function inferEntityRole(field) {
    const txt = mergeTextParts([
      field.context,
      field.sectionText,
      field.formText,
    ...
    if (/\b(reference|referee|guarantor)\b|紹介|保証人/i.test(txt)) return 'reference';

    return null;
  }

  function hasDistinctEntityRolePair(roles) {
    const roleSet = new Set(roles);

    if (roleSet.has('billing') && roleSet.has('shipping')) return true;
    if (roleSet.has('primary') && roleSet.has('secondary')) return true;
    if (roleSet.has('emergency') && (roleSet.has('primary') || roleSet.has('applicant'))) return true;
    if (roleSet.has('guardian') && roleSet.has('applicant')) return true;
    if (roleSet.has('reference') && roleSet.has('applicant')) return true;
  ...
  function areClearlyDifferentPurpose(fields) {
    ...
    const purposeSignatures = unique(fields.map((f) => f.purposeSignature).filter(Boolean));
    const entityRoles = unique(fields.map((f) => inferEntityRole(f)).filter(Boolean));

    if (hasDistinctEntityRolePair(entityRoles)) return true;
