...
async function _assertPublicUrl(url) {
  ...
  try {
    ...
    throw new SsrfGuardError(
      `SSRF guard: DNS resolution failed for ${hostname}`,
    );
  }
  if (!Array.isArray(addresses) || addresses.length === 0) {
    throw new SsrfGuardError(
      `SSRF guard: DNS resolution returned no addresses for ${hostname}`,
    );
  }
...
const AXE_LOCALE_DIR = path.join(
...
function _loadAxeLocale(lang = "en") {
  ...
  const localeId =
    AXE_LOCALE_ALIASES[normalized] ||
    AXE_LOCALE_ALIASES[normalized.split("-")[0]];
...
class AccessibilityService {
  ...
  async _injectAxe(page, logPrefix = "") {
    ...
    const tryInject = async () => {
      const injectWithTimeout = (fn) =>
        Promise.race([
          ...
          new Promise((_, reject) =>
            setTimeout(
              () => reject(new Error("axe injection timed out")),
              INJECT_TIMEOUT_MS,
            ),
          ),
        ]);
      await injectWithTimeout(async () => {
        ...
        const alDir = require("path").dirname(
          require.resolve("@accesslint/core"),
        );
    ...
    try {
      ...
      this._logger.warn(
        `${prefix}axe-core was not available after injection, retrying once: ${err.message}`,
      );
      try {
        await tryInject();
      } catch {
  ...
  async _configureAxeLocale(page, lang = "en", logPrefix = "") {
    ...
    this._logger.info(
      `${prefix}Configured axe-core locale: ${_sanitizeLocaleLang(lang)}`,
    );
  }
  ...
  async _runAxeWithTimeout(page, runOnly, timeoutMs, logPrefix = "", disabledRules = null) {
    ...
    try {
      await page.evaluate(() => {
        window.__ka11yAxeAbort = false;
      });
    } catch (_) {
      /* page may not be ready or mock not configured — ignore */
    }
    ...
    const timeoutPromise = new Promise((_, reject) => {
      timeoutHandle = setTimeout(async () => {
        ...
        try {
          await page.evaluate(() => {
            try {
              window.__ka11yAxeAbort = true;
            } catch (_) {}
            // eslint-disable-next-line no-undef
            if (
              typeof axe !== "undefined" &&
              typeof axe._abortAll === "function"
            ) {
              try {
                axe._abortAll();
              } catch (_) {}
          ...
          this._logger.debug(
            `${prefix}Could not signal axe abort: ${e.message}`,
          );
        }
        reject(
          new Error(`${prefix}axe.run timed out after ${effectiveTimeoutMs}ms`),
        );
      }, effectiveTimeoutMs);
    });
    ...
    try {
      ...
      axeRunPromise.catch((e) => {
        ...
        if (
          ...
          this._logger.debug(
            `${prefix}Late axe.run rejection after browser closed: ${msg}`,
          );
        } else {
      ...
      for (const node of rule.nodes || []) {
        ...
        try {
          ...
          if (withScreenshots && node.boundingBox) {
            node.screenshot = await el
              .screenshot({ encoding: "base64" })
              .catch(() => null);
          }
        } catch (e) {
          this._logger.debug(
            `Element enrichment skipped for "${selector}": ${e.message}`,
          );
        }
      }
    }
  ...
  async analyseUrlReport(url, lang = "en") {
    ...
    try {
      ...
      const pageScreenshot = await page
        ...
        .catch((e) => {
          this._logger.warn(
            `[report] Full-page screenshot failed: ${e.message}`,
          );
          return null;
        });
      ...
      this._logger.info(
        `[report] Generating HTML report — ${findings.length} finding(s).`,
      );
  ...
  async analyze(html, criteriaId = null, lang = "en") {
    ...
    try {
      ...
      const customResults = await runStaticChecks(
        page,
        criteriaId != null ? criteriaId : { lang },
      );
      this._logger.info(
      ...
      return mergeWithAxe(
        mapResults(axeResults, criteriaId, accessLintResults),
        customResults,
      );
    } catch (err) {
    ...
    try {
      ...
      await page
        .waitForFunction(
          ...
          { timeout: POST_NAV_SETTLE_MS + 500 },
          POST_NAV_SETTLE_MS,
        )
        .catch(() => {});
  ...
  async _auditPageFlat(
    ...
    try {
      ...
      try {
        ...
        this._logger.debug(
          `[flat] page.url() read failed for ${url}: ${e.message}`,
        );
      }
      ...
      try {
        ...
        this._logger.warn(
          `[flat] Custom checks failed for ${url}: ${err.message}`,
        );
      }

      const mappingStart = process.hrtime.bigint();
      const allCustomFindings = mapCustomResultsFlat(
        customResults,
        resolvedUrl,
        lang,
      );
      const allowedLevels = _allowedLevels(level);
      let customFindings = allCustomFindings.filter(
        (f) => !f.level || allowedLevels.has(f.level),
      );

      let findings = [
        ...mapResultsFlat(axeResults, resolvedUrl, lang),
        ...customFindings,
      ];
      timings.record({
        stage: "axe_core",
      ...
      if (successCriteriaId) {
        findings = findings.filter(
          (f) =>
            (f.successCriteriaId || f.success_criteria_id || f.wcag_sc) ===
            successCriteriaId,
        );
      }
