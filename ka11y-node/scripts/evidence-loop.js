...
const PYTHON_DIR = path.join(REPO_ROOT, 'ka11y-python');
const FRONTEND_DIR = path.join(REPO_ROOT, 'ka11y-frontend-sdk');
...
const MAX_ATTEMPTS = parseInt(process.env.KA11Y_EVIDENCE_MAX_ATTEMPTS || '3', 10);
const REQUESTED_FRONTEND_PREVIEW_PORT = process.env.KA11Y_FRONTEND_PREVIEW_PORT
  ? parseInt(process.env.KA11Y_FRONTEND_PREVIEW_PORT, 10)
  : null;
...
async function runNodeEvidence(browser) {
  ...
  const shortcutFailFindings = await runCustomCheck(
    ...
    makeHtml(`
      ...
      <button onkeydown="if (event.key === 'x') window.__ka11yEvidence = true;">Shortcut</button>
    `)
  );
  ...
  const shortcutPassFindings = await runCustomCheck(
    ...
    characterKeyShortcutsCheck,
    makeHtml(`
      <button onkeydown="if (event.ctrlKey && event.key === 's') window.__ka11yEvidence = true;">Save</button>
    `)
  );
...
function writeReport(finalResult, attempts) {
  ...
  const lines = [];
  lines.push('# ka11y Evidence Report');
  lines.push('');
...
main().catch((error) => {
  fs.mkdirSync(OUTPUT_DIR, { recursive: true });
  fs.writeFileSync(REPORT_PATH, `# ka11y Evidence Report\n\nExecution failed: ${error.stack || error.message}\n`, 'utf8');
  console.error(error);
  process.exitCode = 1;
});
