...
app.post(`${API_V1}/analyze-accessibility`, (req, res) =>
  accessibilityController.analyze(req, res),
);
app.post(`${API_V1}/analyse-url`, (req, res) =>
  accessibilityController.analyseUrl(req, res),
);
app.post(`${API_V1}/analyse-url/report`, (req, res) =>
  accessibilityController.analyseUrlReport(req, res),
);
app.post(`${API_V1}/analyse-url-flat`, (req, res) =>
  accessibilityController.analyseUrlFlat(req, res),
);
app.post(`${API_V1}/rules/:successCriteriaId/analyse-url`, (req, res) =>
  accessibilityController.analyseRuleUrl(req, res),
);
app.post(`${API_V1}/analyse-url-wcag`, (req, res) =>
  wcagController.analyseUrl(req, res),
);
...
app.get(`${API_V1}/rules-guide`, (req, res) =>
  rulesGuideController.getAll(req, res),
);
app.get(`${API_V1}/rules-guide/:ruleId`, (req, res) =>
  rulesGuideController.getOne(req, res),
);
...
app.get("/rules-guide/:ruleId", (req, res) =>
  rulesGuideController.getOne(req, res),
);
...
app.use((req, res) => {
  res
    .status(404)
    .json({ error: "Not Found", message: `Route ${req.url} does not exist` });
});
...
if (require.main === module) {
  ...
  app.listen(config.port, () => {
    ...
    logger.info(
      `  GET  ${BASE_URL}/api/v1/health                  — Liveness / readiness probe`,
    );
    logger.info(
      `  POST ${BASE_URL}/api/v1/analyze-accessibility   — Run axe-core audit on raw HTML`,
    );
    logger.info(
      `  POST ${BASE_URL}/api/v1/analyse-url             — Crawl a URL and run axe-core audit`,
    );
    logger.info(divider);
    ...
    logger.info(
      `  axe timeout : ${config.axe.timeoutMs / 1000}s  |  default tags: ${config.axe.defaultTags.join(", ")}`,
    );
    logger.info(divider);
