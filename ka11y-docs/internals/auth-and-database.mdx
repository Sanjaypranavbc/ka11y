...
Fourteen tables; Alembic migrations `0001_initial_schema` and
`0002_user_password_hash` (`ka11y-python/alembic/versions/`). UUID primary keys
...
| Table | Purpose |
|-------|---------|
| `users` | Application user. `password_hash` (scrypt, nullable) is set only for accounts that use e-mail + password sign-in; OIDC-only accounts keep it NULL. Never a plaintext password. |
...
Two methods share the same `users` row, allow-list, organization logic and
session cookie:

| Method | Enabled by | How |
|--------|-----------|-----|
| **E-mail + password** (default on) | `KA11Y_PASSWORD_LOGIN=1` | Form on `/login` → `POST /api/v1/auth/password/login`. Passwords are hashed with scrypt (`ka11y/auth/passwords.py`, standard library, self-describing hash string so parameters can be raised later). |
| **Self-service registration** (default on) | `KA11Y_PASSWORD_REGISTRATION=1` | `/register` → `POST /api/v1/auth/password/register`. Only addresses on `KA11Y_ALLOWED_EMAILS` / `_DOMAINS` may register, and an address that already has an account (OIDC or password) is refused with `account_exists`. With registration off, set passwords with `poetry run python scripts/set_password.py someone@kao.com --create` (inside compose: `docker compose exec python python scripts/set_password.py …`). |
| **Bootstrap accounts** (closed pilot) | `KA11Y_BOOTSTRAP_PASSWORD=…` | At start-up (after migrations) every `KA11Y_ALLOWED_EMAILS` address gets an account, and any without a password gets this one. Idempotent and never overwrites a password that already exists. Combine with `KA11Y_PASSWORD_REGISTRATION=0`. |
| **OIDC** (Google, Microsoft Entra, …) | the `KA11Y_OIDC_*` variables | Redirect flow below. Offered on `/login` as "Continue with <provider>" when configured. |

`GET /api/v1/auth/config` tells the UI which methods are on (`oidc`,
`password_login`, `registration`). Password failures are rate-limited per
(client IP, e-mail): 10 failures in 15 minutes → `429 too_many_attempts`
(in-process; use one API replica or accept per-replica counters). A login
attempt for an address that is not on the allow-list is refused even if the
row and password exist, so removing someone from the list locks them out.
Password rules: 8–128 characters, no leading/trailing spaces, nothing else
(NIST-style; length over complexity).

Error codes returned as `{"error": code}` and translated by the UI:
`invalid_credentials` (401), `no_password` (403, OIDC-only account),
`not_allowed`, `account_suspended`, `login_disabled`, `register_disabled`
(403), `weak_password` (400), `account_exists` (409), `too_many_attempts` (429).
...
| `GET /api/v1/auth/me` | cookie | Current user, organization, role, `is_admin` (e-mail on `KA11Y_ADMIN_EMAILS`) |
...
| `POST /api/v1/auth/password/login` | public | `{email, password, remember?, next?}` → `{next}` + cookie |
| `POST /api/v1/auth/password/register` | public | `{email, password, name?, remember?, next?}` → 201 `{next}` + cookie |
...
`/admin` in the UI is for the addresses on `KA11Y_ADMIN_EMAILS` only. The
admin layout (`ka11y-ui/src/app/admin/layout.tsx`) is a server component
that calls `/api/v1/auth/me` with the browser's cookie and returns **404**
for anyone else, so the console's existence is not advertised. API routes
that must be admin-only use `Depends(require_admin)` from
`ka11y/auth/dependencies.py` (403 for signed-in non-admins).
