...
You provide a URL. ka11y launches two specialised engines — one that analyses the **rendered visual page** and one that analyses the **underlying code structure**. Both engines crawl the same URL in parallel (unless the Node engine is disabled via the `run_node_audit` flag), then their findings are merged into a single unified report.
...
ka11y crawls multiple same-domain pages and attributes every finding to the page
