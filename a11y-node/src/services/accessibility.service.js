...
class AccessibilityService {
   ...
   *   1. Set a window.__a11yAxeAbort flag so callers / future axe versions
  ...
  async _runAxeWithTimeout(page, runOnly, timeoutMs, logPrefix = '') {
    ...
    try {
      await page.evaluate(() => { window.__a11yAxeAbort = false; });
    ...
    const timeoutPromise = new Promise((_, reject) => {
      timeoutHandle = setTimeout(async () => {
        ...
        try {
          await page.evaluate(() => {
            try { window.__a11yAxeAbort = true; } catch (_) { }
            // eslint-disable-next-line no-undef
