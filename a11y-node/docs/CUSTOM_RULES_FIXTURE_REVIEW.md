...
Date: 2026-04-13
Scope: `a11y-node/src/custom-checks`, `a11y-node/src/utils`, `a11y-node/src/controllers`, `a11y-python/a11y`
...
This document focuses on what needs to be fixed next, not on restating the existing rule-by-rule analysis in `a11y-node/docs/RULE_ANALYSIS.md`.
...
- `a11y-node/src/controllers/accessibility.controller.js` only accepts `lang` for `analyseUrlFlat`.
- `a11y-node/src/utils/wcagMetadata.js` builds `WCAG_NAMES` from `getRules('en')` once at startup.
- `a11y-node/src/utils/axeResultMapper.js` uses `_criterionName()` from the English map, while `_suggestedFix()` reads the requested locale.
- Python already localizes finding names at finding-construction time via `get_wcag_names(_lang)` in `a11y-python/a11y/api/v1/combined/findings.py`.
...
- `i18n/rules.yml` and `a11y-node/i18n/rules.yml` currently match.
- `a11y-node/i18n/locales/ja.yml` differs from `i18n/locales/ja.yml`.
- `a11y-node/i18n/rules.yml` differs from `a11y-python/i18n/rules.yml`.
- `a11y-node/i18n/locales/ja.yml` and `a11y-python/i18n/locales/ja.yml` currently match.
...
```text
a11y-node/tests/fixtures/custom-rules/en/
a11y-node/tests/fixtures/shared/
a11y-node/tests/fixtures/expected/en/
```
...
```text
a11y-node/tests/fixtures/custom-rules/ja/
a11y-node/tests/fixtures/expected/ja/
```
...
- keep `a11y.i18n.loader` but resolve from the shared root i18n path

Environment override suggestion:

- `A11Y_SHARED_CONFIG_DIR`
- `A11Y_SHARED_I18N_DIR`

This removes the current per-service copies under `a11y-node/i18n` and `a11y-python/i18n`.
...
1. `a11y-node/tests/custom-checks/orientation.check.test.js`
2. `a11y-node/tests/custom-checks/pronunciation.check.test.js`
